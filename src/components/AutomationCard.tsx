...
const automations = [
  {
    ...
    description: "Auto-transfer ₹50 to emergency fund on every food order",
    emoji: "🍕",
    active: true,
    saved: 650,
  },
  {
    ...
    description: "Get a nudge when weekend spending exceeds ₹1,000",
    emoji: "🛡️",
    active: true,
    saved: 2200,
  },
...
const AutomationCard = () => {
  const [automationStates, setAutomationStates] = useState(
    automations.reduce((acc, a) => ({ ...acc, [a.id]: a.active }), {} as Record<number, boolean>)
  );
    ...
    >
      ...
      <div className="space-y-3">
        {automations.map((auto, index) => (
          ...
          >
            <span className="text-2xl">{auto.emoji}</span>
            <div className="flex-1 min-w-0">
              ...
              {automationStates[auto.id] && auto.saved > 0 && (
            ...
            <button
              ...
              className={`relative w-11 h-6 rounded-full transition-colors duration-300 ${
                automationStates[auto.id] ? "bg-accent" : "bg-muted-foreground/30"
              }`}
              role="switch"
              aria-checked={automationStates[auto.id]}
              aria-label={`Toggle ${auto.title}`}
            >
              <motion.div
                animate={{ x: automationStates[auto.id] ? 20 : 2 }}
